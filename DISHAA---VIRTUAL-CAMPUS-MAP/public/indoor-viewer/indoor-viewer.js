...
function init() {
    ...
    const initialBlock = (urlParams.get('block') || 'A').toUpperCase();
    const initialFloorStr = urlParams.get('floor');
    const initialFloor = initialFloorStr !== null ? parseInt(initialFloorStr, 10) : (initialBlock === 'B' ? 1 : 0);
    switchBlockAndFloor(initialBlock, initialFloor);
    animate();
}
...
// Multi-Route Indoor Navigation State
let currentIndoorRoutes = [];
let activeIndoorRouteIndex = 0;

function runDijkstraIndoor(config, startNodeId, destNodeId, edgePenalties = {}) {
    const graph = {};
    ...
    config.graphEdges.forEach(([u, v]) => {
        ...
        if (p1 && p2) {
            ...
            const penalty = (edgePenalties[`${u}-${v}`] || 0) + (edgePenalties[`${v}-${u}`] || 0);
            dist += penalty;
            graph[u][v] = dist;
            graph[v][u] = dist;
        }
    ...
    if (path.length > 0 && path[0] === startNodeId) {
        let unweightedDist = 0;
        for (let i = 0; i < path.length - 1; i++) {
            const p1 = config.graphNodes[path[i]];
            const p2 = config.graphNodes[path[i + 1]];
            if (p1 && p2) {
                unweightedDist += Math.sqrt(Math.pow(p1.x - p2.x, 2) + Math.pow(p1.z - p2.z, 2));
            }
        }
        return { path, distance: unweightedDist };
    }
    return null;
}
...
function findRoute() {
    ...
    currentIndoorRoutes = [];
    activeIndoorRouteIndex = 0;
    const edgePenalties = {};

    const routeConfigs = [
        { id: 'in-route-1', name: 'Shortest Route', color: '#38bdf8', isShortest: true, penaltyAmount: 250 },
        { id: 'in-route-2', name: 'Alternative 2', color: '#a78bfa', isShortest: false, penaltyAmount: 500 },
        { id: 'in-route-3', name: 'Alternative 3', color: '#34d399', isShortest: false, penaltyAmount: 1000 },
    ];

    for (let i = 0; i < routeConfigs.length; i++) {
        const rc = routeConfigs[i];
        const res = runDijkstraIndoor(config, startNodeId, destNodeId, edgePenalties);
        if (!res) break;

        const isDuplicate = currentIndoorRoutes.some(ex => {
            if (Math.abs(ex.distance - res.distance) < 20) return true;
            const setA = new Set(res.path);
            const setB = new Set(ex.path);
            let match = 0;
            setA.forEach(n => { if (setB.has(n)) match++; });
            return (match / Math.max(setA.size, setB.size)) > 0.8;
        });

        for (let k = 0; k < res.path.length - 1; k++) {
            const u = res.path[k];
            const v = res.path[k + 1];
            edgePenalties[`${u}-${v}`] = (edgePenalties[`${u}-${v}`] || 0) + rc.penaltyAmount;
            edgePenalties[`${v}-${u}`] = (edgePenalties[`${v}-${u}`] || 0) + rc.penaltyAmount;
        }

        if (!isDuplicate || i === 0) {
            currentIndoorRoutes.push({
                id: rc.id,
                name: i === 0 ? `Shortest Route (${(res.distance / 100).toFixed(1)}m)` : `Alternative ${i + 1} (${(res.distance / 100).toFixed(1)}m)`,
                path: res.path,
                distance: res.distance,
                isShortest: rc.isShortest,
                color: rc.color,
            });
        }
    }

    if (currentIndoorRoutes.length > 0) {
        drawMultipleIndoorRoutes();
        generateInstructions(currentIndoorRoutes[0].path, startId, destId, currentIndoorRoutes[0].distance);
    } else {
        alert('No pathway found between these rooms.');
    }
}

function selectIndoorRoute(index) {
    if (index < 0 || index >= currentIndoorRoutes.length) return;
    activeIndoorRouteIndex = index;
    const selected = currentIndoorRoutes[index];
    drawMultipleIndoorRoutes();
    generateInstructions(selected.path, startSelect.value, destSelect.value, selected.distance);
}

function drawMultipleIndoorRoutes() {
    clearRouteVisuals();
    updateStartDestPins();

    if (!currentIndoorRoutes || currentIndoorRoutes.length === 0 || !currentModel) return;

    const config = BLOCK_CONFIGS[currentBlock].floorConfigs[currentFloor];

    // Render each candidate route in 3D/2D space
    currentIndoorRoutes.forEach((rt, idx) => {
        const isActive = (idx === activeIndoorRouteIndex);
        const rawPoints = [];

        rt.path.forEach(nodeId => {
        ...
        if (!isActive) {
            // Render Dimmed Inactive Alternative Route Path
            const tubeGeo = new THREE.TubeGeometry(curvePath, 80, 4.0, 6, false);
            const tubeMat = new THREE.MeshBasicMaterial({
                color: new THREE.Color(rt.color || '#94a3b8'),
                transparent: true,
                opacity: 0.35,
                depthTest: false
            });
            const inactiveMesh = new THREE.Mesh(tubeGeo, tubeMat);
            inactiveMesh.renderOrder = 990;
            inactiveMesh.userData = { isAlternativeRoute: true, routeIndex: idx };
            routeGroup.add(inactiveMesh);
        } else {
            // Render Active Route with Glowing Chevrons & Airport Runway Track
...
function generateInstructions(path, startId, destId, totalDist) {
    ...
    // Multi-Route Selector Tabs in Indoor Directions Sidebar
    if (currentIndoorRoutes && currentIndoorRoutes.length > 1) {
        html += `<div style="display: flex; gap: 6px; margin-bottom: 12px; padding: 4px; background: rgba(241, 245, 249, 0.8); border-radius: 10px; border: 1px solid #e2e8f0;">`;
        currentIndoorRoutes.forEach((rt, idx) => {
            const isActive = (idx === activeIndoorRouteIndex);
            const borderCol = rt.color || '#38bdf8';
            html += `
                <button onclick="window.selectIndoorRoute(${idx})" style="flex: 1; padding: 6px 4px; border-radius: 8px; font-size: 11px; font-weight: 700; cursor: pointer; border: 1px solid ${isActive ? borderCol : '#cbd5e1'}; background: ${isActive ? '#ffffff' : 'transparent'}; color: ${isActive ? '#0f172a' : '#64748b'}; shadow: ${isActive ? '0 1px 2px rgba(0,0,0,0.05)' : 'none'};">
                    <span style="display: inline-block; width: 6px; height: 6px; border-radius: 50%; background: ${borderCol}; margin-right: 4px;"></span>
                    Route ${idx + 1} ${rt.isShortest ? '<span style="font-size: 8px; color: #2563eb; background: #dbeafe; padding: 1px 3px; border-radius: 3px;">FASTEST</span>' : ''}
                </button>
            `;
        });
        html += `</div>`;
...
window.selectIndoorRoute = selectIndoorRoute;
